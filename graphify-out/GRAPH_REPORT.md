...
### Community 0 - "GoBoard"
Cohesion: 0.06
Nodes (19): _copy_game_tree(), GameNode, GoBoard, parse_sequence(), load_tk_image(), Load an image as a Tk image, preferring Pillow for broad format support., 依 board_shell 實際尺寸重新縮放外框背景圖片（cover 模式：填滿裁切）。 由 board_shell 的 <Configure>…, 動態生成歷史落子紀錄，不會再因為提子而消失，確保 AI 判斷正確 (+11 more)
...
Cohesion: 0.20
Nodes (14): json, opencc, get_publisher_from_model_id(), group_models_by_publisher(), 從 model_id 拆出 publisher（第一個 '/' 之前的部分）。 無 '/' 的 model_id 歸類為 "unknown"，確保 UI…, 將 model_id 清單依 publisher 分組，回傳 {publisher: [model_id, ...]}。 保持各 publisher 內…, discover_openrouter_models(), get_publisher_from_model_id() (+6 more)

### Community 9 - "refresh_tab_bar"
Cohesion: 0.11
Nodes (30): _capture_board_snapshot(), _close_tab_silently(), hydrate_active_session(), load_sgf_file(), on_close_tab_click(), on_copy_tab_click(), _on_find_tab_changed(), on_load_sgf_click() (+22 more)

### Community 10 - "rules.py"
Cohesion: 0.16
Nodes (13): dataclasses, math, calculate_area_score(), get_display_komi_from_sgf(), get_katago_komi(), get_rule_preset(), normalize_rule_id(), Supported KataGo rules and per-game analysis setting helpers. (+5 more)

### Community 11 - "get_active_analysis_settings"
Cohesion: 0.24
Nodes (7): normalize_analysis_settings(), get_active_analysis_settings(), GoDataFilter, 【改進】從快取中查詢上一手 (turn-1) 的分析結果，取出勝率和目數作為基準 Args: turn: 當前手數 analyzer:…, Edit the active tab's rules and the default for newly created tabs., show_rules_settings_dialog(), apply_rules()

### Community 12 - "version.py"
Cohesion: 0.24
Nodes (12): argparse, Path, re, main(), Application version helpers for AI Go Teacher. Run this file to update every…, Return the numeric tuple used by PyInstaller's VSVersionInfo., _replace_once(), sync_version() (+4 more)
...
### Community 19 - "OllamaProvider"
Cohesion: 0.16
Nodes (3): OllamaProvider, get_ollama_manager(), OllamaModelInfo

### Community 20 - "_handle_score_estimate_result"
Cohesion: 0.21
Nodes (11): count_captured_prisoners(), _handle_score_estimate_result(), on_close_score_estimate_click(), on_score_estimate_click(), 依歷史落子順序重播棋局，計算雙方已提取的對方棋子數。 回傳 {"black": 黑方提掉的白子數, "white": 白方提掉的黑子數}。, show_score_estimate_popup(), close_popup(), _start_score_estimate_query() (+3 more)
...
Cohesion: 0.15
Nodes (10): create_dev_menu(), _create_info_section(), _create_katago_section(), _create_labeled_row(), export_diagnostic_report(), 建立診斷資訊視窗中的單列 label/value。, 匯出診斷報告到 diagnostics/diagnostic_report.txt。, Return Dev menu items for the themeable menu bar. (+2 more)

### Community 31 - "materialize_bundled_runtime_file"
Cohesion: 0.06
Nodes (45): _delete_api_key(), delete_nvidia_api_key(), delete_openrouter_api_key(), Delete one application credential, treating a missing credential as success., Delete the NVIDIA API key owned by this application., Delete the OpenRouter API key owned by this application., _build_diagnostic_report_text(), _clear_selected_runtime_data() (+37 more)

### Community 36 - "chat_sandbox.py"
Cohesion: 0.19
Nodes (11): copy, os, pathlib, pil, sys, 主題切換時由主程式呼叫：重繪所有開著的聊天視窗。, 讀取主程式目前主題（light/dark），失敗時退回 dark。 ConfigService 需要有狀態的 backend，無法獨立重新建立實例，…, refresh_open_windows() (+3 more)

### Community 38 - "start_analyzer_async"
Cohesion: 0.12
Nodes (27): auto_analyze(), get_config_display_name(), get_model_display_name(), is_analyzer_ready(), on_analyze_button_click(), poll_ai(), 分析整盤棋並回傳每手的勝率列表 (複用全局 KataGo analyzer，支援取消與進度回報), 直接使用記憶體中的數據更新 UI，並將所有分析結果保存到快取以供後續比較使用 (+19 more)

### Community 39 - "main_v3.py"
Cohesion: 0.06
Nodes (36): collections, ctypes, dotenv, hashlib, itertools, logging, matplotlib, matplotlib_backends_backend_tkagg (+28 more)
...
Cohesion: 0.08
Nodes (22): apply_theme(), recolor(), build_menu_bar(), set_language(), toggle_dev(), on_closing(), Refresh controls that expose the continuous-analysis state., Expose semantic theme tokens to legacy drawing code in this module. (+14 more)
...
### Community 47 - "t"
Cohesion: 0.10
Nodes (31): change_config_path(), change_katago_path(), change_model_path(), create_katago_startup_popup(), update_elapsed(), detect_ollama_installed(), open_feedback_form(), _open_folder() (+23 more)
...
Cohesion: 0.22
Nodes (9): _confirm_and_download_ollama_model(), _create_ollama_model_row(), on_row_click(), _download_ollama_model(), on_download_complete(), update_progress(), _load_ollama_icon(), 為 Ollama 模型創建一個選擇行。 - 已下載（available）：點擊直接選中 - 雲端（cloud）：點擊直接選中，不顯示下載 -… (+1 more)
...
- **Why does `t()` connect `t` to `GoBoard`, `_show_llm_selection_dialog`, `refresh_tab_bar`, `get_active_analysis_settings`, `TabManager`, `BranchTreeView`, `_handle_score_estimate_result`, `I18n`, `normalize_api_key`, `plot_window`, `show_system_info_dialog`, `materialize_bundled_runtime_file`, `start_analyzer_async`, `main_v3.py`, `build_menu_bar`, `show_game_info_dialog`, `_show_close_tab_dialog`, `_download_ollama_model`, `create_path_row`, `show_find_dialog`?**
