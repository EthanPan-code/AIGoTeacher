...
## Corpus Check
- 25 files · ~44,525 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 651 nodes · 1252 edges · 41 communities (32 shown, 9 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 83 edges (avg confidence: 0.53)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `6399353d`
...
### Community 0 - "t"
Cohesion: 0.12
Nodes (32): build_menu_bar(), change_config_path(), change_katago_path(), change_model_path(), create_katago_startup_popup(), get_config_display_name(), get_model_display_name(), on_analyze_button_click() (+24 more)

### Community 1 - "GoBoard"
Cohesion: 0.06
Nodes (21): auto_analyze(), GameNode, GoBoard, is_analyzer_ready(), load_tk_image(), new_game(), _on_board_shell_configure(), on_mouse_wheel() (+13 more)
...
Cohesion: 0.19
Nodes (16): create_dev_menu(), ensure_runtime_dir(), export_diagnostic_report(), get_executable_dir(), get_katago_runtime_overrides(), get_runtime_data_root(), get_runtime_file_path(), is_frozen_app() (+8 more)
...
Cohesion: 0.10
Nodes (13): get_commentary_from_cache(), GoDataFilter, KataGoAnalyzer, on_closing(), poll_ai(), 【改進】從快取中查詢上一手 (turn-1) 的分析結果，取出勝率和目數作為基準                  Args:             t, 分析整盤棋並回傳每手的勝率列表 (複用全局 KataGo analyzer，支援取消與進度回報), 直接使用記憶體中的數據更新 UI，並將所有分析結果保存到快取以供後續比較使用 (+5 more)
...
Cohesion: 0.11
Nodes (7): ProviderFactory, Reverse lookup: display name → model ID.          Returns None when the displa, Return [(display_name, model_id), ...] for UI widgets.          The list follo, 向 NIM 端點探索可用模型，失敗時降級至內建清單。          回傳 (model_ids, used_fallback, error_messag, 從 model_id 清單取出 publisher 清單（已排序、去重）。, 取得指定 publisher 下的 model_id 清單（保持原始順序）。, 從 model_id 拆出 publisher（供 UI 還原選擇用）。
...
Cohesion: 0.23
Nodes (13): get_nvidia_api_key(), get_openrouter_api_key(), normalize_api_key(), Trim whitespace and common quote wrappers from API key values., Read NVIDIA API key from keyring first, then environment variables., Store NVIDIA API key in the OS keyring. Does not write to .env., Read OpenRouter API key from keyring first, then environment variables., Store OpenRouter API key in the OS keyring. Does not write to .env. (+5 more)
...
Cohesion: 0.15
Nodes (14): _confirm_and_download_ollama_model(), _create_info_section(), _create_katago_section(), _create_labeled_row(), _create_ollama_model_row(), _download_ollama_model(), _load_ollama_icon(), needs_first_run_onboarding() (+6 more)
...
Cohesion: 0.25
Nodes (7): Return the human-readable display name for a model ID.          Falls back to, Open the LLM Chat Sandbox window for provider connectivity testing., rebuild_menu_bar(), refresh_language(), render_winrate_text(), show_chat_sandbox(), update_llm_model_label()
...
### Community 40 - ".get_nim_publisher_for_model"
Cohesion: 0.67
Nodes (3): _open_folder(), 顯示診斷報告匯出完成訊息與開啟資料夾按鈕。, _show_diagnostic_export_success()
...
- **Why does `GoBoard` connect `GoBoard` to `t`, `LLMChatWindow`, `_handle_score_estimate_result`, `BranchTreeView`, `ConfigService`, `ProviderFactory`, `version.py`, `main_v3.py`, `set_winrate_text`?**
  _High betweenness centrality (0.115) - this node is a cross-community bridge._
- **Why does `ProviderFactory` connect `ProviderFactory` to `GoBoard`, `show_first_run_onboarding_dialog`, `OllamaProvider`, `BranchTreeView`, `KataGoAnalyzer`, `main_v3.py`, `version.py`, `github_provider.py`, `main_v3.py`, `NvidiaProvider`?**
  _High betweenness centrality (0.115) - this node is a cross-community bridge._
- **Why does `LLMChatWindow` connect `LLMChatWindow` to `GoBoard`, `show_first_run_onboarding_dialog`, `BranchTreeView`, `KataGoAnalyzer`, `version.py`, `main_v3.py`?**
