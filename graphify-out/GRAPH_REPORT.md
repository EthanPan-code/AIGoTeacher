...
## Corpus Check
- 29 files · ~77,006 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 6 file(s) not represented in the graph (top: .spec 1, (none) 1, .cfg 1)

## Summary
- 1096 nodes · 2249 edges · 57 communities (46 shown, 11 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 140 edges (avg confidence: 0.86)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `38b3a7ed`
...
9. `plot_window()` - 24 edges
10. `OllamaProvider` - 23 edges
...
Cohesion: 0.06
Nodes (39): ProviderFactory, Return the human-readable display name for a model ID. Falls back to the raw…, Reverse lookup: display name → model ID. Returns None when the display name is…, Return [(display_name, model_id), ...] for UI widgets. The list follows the…, 向 NIM 端點探索可用模型，失敗時降級至內建清單。 回傳 (model_ids, used_fallback, error_message)： -…, 從 model_id 清單取出 publisher 清單（已排序、去重）。, 取得指定 publisher 下的 model_id 清單（保持原始順序）。, 從 model_id 拆出 publisher（供 UI 還原選擇用）。 (+31 more)
...
Cohesion: 0.09
Nodes (9): ConfigService, Small wrapper around persisted UI settings., Migrate settings from the removed GitHub Models provider., detect_system_theme(), normalize_theme(), Application color themes and Windows system-theme resolution., Return the Windows theme at process startup; safely fall back to light., resolve_theme() (+1 more)
...
### Community 9 - "_close_tab_silently"
Cohesion: 0.17
Nodes (18): _capture_board_snapshot(), _close_tab_silently(), hydrate_active_session(), _on_find_tab_changed(), on_load_sgf_click(), on_new_tab_click(), on_tab_click(), Stop the active continuous query and clear its routing state. (+10 more)

### Community 10 - "resource_path"
Cohesion: 0.14
Nodes (14): detect_ollama_installed(), open_feedback_form(), _open_folder(), 在 Windows 上抑制子進程彈出的主控台視窗。 隱藏終端機 (windowed) 模式下，子進程預設會繼承一個可見的主控台， 即使…, 檢查系統是否能執行 `ollama --version`，回傳 (installed: bool, version_or_none), 顯示簡單的 Ollama 安裝引導對話框（包含開啟下載頁與重新檢測）。, 顯示診斷報告匯出完成訊息與開啟資料夾按鈕。, resource_path() (+6 more)
...
Cohesion: 0.05
Nodes (36): math, calculate_area_score(), get_display_komi_from_sgf(), get_katago_komi(), get_rule_preset(), normalize_analysis_settings(), normalize_rule_id(), Supported KataGo rules and per-game analysis setting helpers. (+28 more)
...
Cohesion: 0.21
Nodes (4): tkinter, FakeMenuBar, Themeable, Tk-only application menu bar. This deliberately does not use native…, A small menu system built from Frames and Buttons. Menu definitions are plain…
...
### Community 22 - "I18n"
Cohesion: 0.10
Nodes (19): find_preset_tone(), get_all_tones(), get_tone_description(), get_tone_display_name(), get_tone_prompt(), Single-block LLM prompt templates for AI Go teacher commentary. The application…, Return the preset prompt in the requested UI language., Return the tone if prompt is an untouched preset, otherwise ``None``. (+11 more)
...
Cohesion: 0.17
Nodes (20): add_full_game_commentary_to_cache(), get_full_game_commentary_from_cache(), plot_window(), apply_chart_mode(), build_full_game_summary_data(), collect_top_moves(), copy_summary_text(), format_move_summary() (+12 more)
...
Cohesion: 0.11
Nodes (15): _build_diagnostic_report_text(), create_dev_menu(), _create_info_section(), _create_katago_section(), _create_labeled_row(), export_diagnostic_report(), _get_newest_log_file(), 建立診斷資訊視窗中的單列 label/value。 (+7 more)

### Community 31 - "get_runtime_data_root"
Cohesion: 0.20
Nodes (17): ensure_runtime_dir(), get_executable_dir(), get_katago_runtime_overrides(), get_runtime_data_root(), get_runtime_file_path(), is_frozen_app(), iter_dotenv_paths(), _iter_log_candidates() (+9 more)
...
Cohesion: 0.16
Nodes (16): auto_analyze(), is_analyzer_ready(), on_analyze_button_click(), poll_ai(), 分析整盤棋並回傳每手的勝率列表 (複用全局 KataGo analyzer，支援取消與進度回報), 直接使用記憶體中的數據更新 UI，並將所有分析結果保存到快取以供後續比較使用, Refresh controls that expose the continuous-analysis state., Refresh a known static teacher prompt without touching LLM output. (+8 more)

### Community 39 - "main_v3.py"
Cohesion: 0.06
Nodes (34): collections, ctypes, dotenv, hashlib, itertools, logging, matplotlib, matplotlib_backends_backend_tkagg (+26 more)
...
Cohesion: 0.10
Nodes (15): apply_theme(), recolor(), build_menu_bar(), toggle_dev(), on_closing(), Expose semantic theme tokens to legacy drawing code in this module., Show a guarded, itemized dialog for deleting application runtime data., Apply a configured theme to existing widgets without restarting. (+7 more)
...
### Community 47 - "t"
Cohesion: 0.18
Nodes (20): change_config_path(), change_katago_path(), change_model_path(), create_katago_startup_popup(), update_elapsed(), get_config_display_name(), get_model_display_name(), 重新初始化分析器（關閉舊進程，建立新進程） (+12 more)
...
### Community 49 - "_copy_game_tree"
Cohesion: 0.17
Nodes (12): _copy_game_tree(), on_close_tab_click(), on_copy_tab_click(), 複製指定分頁的棋局內容，但保持目前 active 分頁不變。, 關閉分頁：dirty 時彈出 三選一（儲存 / 不儲存 / 取消）。 為了在關閉流程內支援 "儲存" 動作，會先暫時把這個分頁切為 active， 讓既有的…, 彈出儲存 / 不儲存 / 取消 三選一對話框，回傳 'save' / 'discard' / 'cancel'。, 遞迴複製 GameNode 樹，不複製 parent（避免循環引用）。 回傳新樹的 root，所有節點的 parent 在複製後重新串接。, save_game_as_sgf() (+4 more)

### Community 50 - "_clear_selected_runtime_data"
Cohesion: 0.20
...
### Community 51 - "_download_ollama_model"
Cohesion: 0.20
...
### Community 55 - "show_appearance_settings_dialog"
Cohesion: 0.38
Nodes (5): show_appearance_settings_dialog(), create_image_row(), browse_image(), use_default(), update_preview()

### Community 56 - "show_winrate_chart"
Cohesion: 0.50
Nodes (5): refresh_nim_models_async(), show_winrate_chart(), on_cancel(), task(), update_progress()
...
- **Why does `LLMChatWindow` connect `LLMChatWindow` to `_show_llm_selection_dialog`, `chat_sandbox.py`, `._start_generation`, `main_v3.py`, `._active_conversation`, `._refresh_conversation_list`, `._tr`, `.__init__`, `._build_ui`, `._apply_chat_palette_swap`?**
  _High betweenness centrality (0.155) - this node is a cross-community bridge._
- **Why does `GoBoard` connect `GoBoard` to `BranchTreeView`, `KataGoAnalyzer`, `main_v3.py`?**
  _High betweenness centrality (0.097) - this node is a cross-community bridge._
- **Why does `t()` connect `t` to `GoBoard`, `_show_llm_selection_dialog`, `_close_tab_silently`, `resource_path`, `materialize_bundled_runtime_file`, `KataGoAnalyzer`, `TabManager`, `BranchTreeView`, `serialize_game_context`, `I18n`, `normalize_api_key`, `plot_window`, `show_system_info_dialog`, `get_runtime_data_root`, `refresh_language`, `main_v3.py`, `build_menu_bar`, `show_game_info_dialog`, `_copy_game_tree`, `_clear_selected_runtime_data`, `_download_ollama_model`, `create_path_row`, `show_find_dialog`, `show_appearance_settings_dialog`, `show_winrate_chart`?**
